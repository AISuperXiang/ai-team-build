...
3. 生成前必须形成 `team-spec.json`，并补齐领域知识、能力矩阵、验收场景和数据契约；不得直接手写大量产物。
4. 自然语言目标可先调用 `scripts/synthesize-team-spec.js --goal <目标>` 选择 domain pack 并生成规格。
...
10. 输出生成路径、角色数量、命令数量、工作流数量、内容深度、验收资产、Adapter、验证结果、评分结论、升级建议和风险约束。
